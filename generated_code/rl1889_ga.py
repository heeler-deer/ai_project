    ...
    for city in range(num_cities):
        if not visited[city]:
            if distances[current_city][city] == 0:
                priorities[city] = -np.inf
            else:
                dynamic_cost = 1
                for i in range(num_cities):
                    if i != city:
                        dynamic_cost *= 1 / (1 + distances[city][i])

                weighted_sum = np.sum(distances[city])
                distance_priority = -1 / distances[current_city][city]
                dynamic_cost = 1 / (1 + weighted_sum + 1)
