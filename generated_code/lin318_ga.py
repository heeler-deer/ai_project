...
def priority(current_city, distances, visited):
    ...
    """Further improved version of priority calculation with additional factors."""
    ...
    for city in range(num_cities):
        if not visited[city]:
            ...
            relative_distance = distances[current_city][city] / avg_distance

            if dynamic_cost > 0.5:
                priorities[city] = distance_priority * (dynamic_cost + 0.5)
            else:
                priorities[city] = distance_priority * (dynamic_cost - 0.2)

            # Add additional complexity based on relative_distance
            if relative_distance < 1:
                priorities[city] *= 1.2  # Favor closer cities
            elif relative_distance > 1:
                priorities[city] *= 0.8  # Discourage distant cities

            # Penalize based on the number of already visited cities nearby
            nearby_visited_count = sum(visited[max(0, city-1):min(num_cities, city+2)])
            priorities[city] *= (1 - 0.1 * nearby_visited_count)  # Less penalty for fewer nearby visits
...
filepath = ['/content/ai_project/sample_data/lin318.tsp']
for i in filepath:
  prepare_dataset(i)
