...
def priority(current_city, distances, visited):
    ...
    for city in range(num_cities):
        ...
        priority = distance * dynamic_cost
        priorities[city] = 1.5 * priority if current_city == 0 else priority
        priorities[city] = np.inf if priority < 0 else priority
        priorities[city] = 100 if priority > 100 else priority
...
filepath = ['/content/ai_project/sample_data/rl1889.tsp']
for i in filepath:
  prepare_dataset(i)
