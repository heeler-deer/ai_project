...
def priority(current_city, distances, visited):
    ...
    for city in range(num_cities):
        ...
        total_distance_from_city = np.sum(distances[city])
        dynamic_cost = 1 / (1 + total_distance_from_city) if total_distance_from_city > 0 else 1

        population_factor = np.random.uniform(0.5, 1.5)
        resources_factor = np.random.uniform(0.8, 1.2)

        priorities[city] = distance_priority * dynamic_cost * population_factor * resources_factor

    max_priority = np.max(priorities[~np.isinf(priorities)])
    priorities = priorities / max_priority if max_priority != 0 else priorities
