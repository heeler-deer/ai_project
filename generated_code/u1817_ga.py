...
filepath = ['/content/ai_project/sample_data/u1817.tsp']
for i in filepath:
  prepare_dataset(i)
...
def priority(current_city, distances, visited):
    ...
    num_cities = len(distances)
    priorities = np.zeros(num_cities)  # Start with zeros; higher means higher priority
    total_distance = np.sum(distances[current_city])  # Total distance from current city

    for city in range(num_cities):
        if visited[city]:
            priorities[city] = -np.inf  # Ensure visited cities have the lowest priority
            continue

        # Inverse of distance (closer cities have higher priority)
        distance_priority = -distances[current_city][city] if distances[current_city][city] > 0 else np.inf

        # Additional heuristic: dynamic cost based on sum of distances from this city
        dynamic_cost = 1 / (1 + np.sum(distances[city])) if np.sum(distances[city]) > 0 else 0

        # Factor in the number of connections (assumed symmetric adjacency)
        connections = np.count_nonzero(distances[city])  # Number of direct connections to other cities
        connection_factor = connections / (num_cities - 1) if num_cities > 1 else 0  # Normalized connection factor

        # Combine metrics into a composite priority value
        priorities[city] = distance_priority * dynamic_cost * (1 + connection_factor)

        # Adjust priorities dynamically based on total distance from the current city
        if total_distance < 100:  # Arbitrary threshold for simplistic behavior adjustment
            priorities[city] *= 1.5  # Higher priority in closer range

    # Normalize priorities to ensure they are within a usable range
