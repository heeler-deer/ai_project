...
def priority(current_city, distances, visited):
    ...
    for city in range(num_cities):
        if not visited[city]:
            ...
            if city % 2 == 0:
                priorities[city] = distance_priority * dynamic_cost * 1.5
            elif city % 3 == 0:
                priorities[city] = distance_priority * dynamic_cost * 2
            else:
                priorities[city] = distance_priority * dynamic_cost

            for i in range(num_cities):
                if i != city:
                    multiplier = 1.2 if i % 2 != 0 else 1.3
                    priorities[i] += distance_priority * dynamic_cost * multiplier
                    for j in range(num_cities):
                        if j != city and j != i:
                            priorities[j] += 2 * distance_priority * dynamic_cost * 1.3
                            priorities[j] += distance_priority * dynamic_cost * 1.1
...
filepath = ['/content/ai_project/sample_data/fli417.tsp']
for i in filepath:
  prepare_dataset(i)
