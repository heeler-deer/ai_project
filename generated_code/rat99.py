...
def priority(current_city, distances, visited):
    ...
    for city in range(num_cities):
        if not visited[city]:
            ...
            additional_factor = np.random.uniform(0, 1)
            priorities[city] = distance_priority * dynamic_cost * additional_factor

            if distances[current_city][city] < 5:
                priorities[city] *= 2

            for i in range(num_cities):
                if distances[city][i] < 10:
                    priorities[city] += 0.5

            if distances[current_city][city] > 15:
                priorities[city] *= 1.5
            elif distances[current_city][city] > 10:
                priorities[city] *= 1.2
            else:
                priorities[city] *= 1.0

            for d in range(num_cities):
                if distances[current_city][d] < 3:
                    priorities[d] += 0.8
                elif distances[current_city][d] > 20:
                    priorities[d] -= 0.5
...
filepath = ['/content/ai_project/sample_data/rat99.tsp']
for i in filepath:
  prepare_dataset(i)
