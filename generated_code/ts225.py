    ...
    for city in range(num_cities):
        if not visited[city]:
            ...
            if distances[current_city][city] > 50:
                distance_priority *= 1.5
            else:
                distance_priority *= 0.5

            for i in range(num_cities):
                if distances[current_city][i] < distances[current_city][city]:
                    distance_priority *= 0.9
                elif distances[current_city][i] == distances[current_city][city]:
                    distance_priority *= 0.8
                else:
                    distance_priority *= 1.1
...
filepath = ['../smaples_data/ts225.tsp']
for i in filepath:
  prepare_dataset(i)
